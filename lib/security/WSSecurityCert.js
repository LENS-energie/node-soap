...
WSSecurityCert.prototype.postProcess = function(xml, envelopeKey) {
  ...
  var secHeader = wsseSecurityHeaderTemplate({
    ...
    includeTimestamp: this.options.includeTimestamp !== false ? true : false
  });

  var xmlWithSec = insertStr(secHeader, xml, xml.indexOf('</' + envelopeKey + ':Header>'));

  var references = this.options.includeReferences !== false ? ["http://www.w3.org/2000/09/xmldsig#enveloped-signature",
